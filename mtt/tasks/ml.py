...
Example usage:
```
law run mtt.MLOptimizer --version prod1 --ml-models dense_3x64,dense_3x128,dense_3x256,dense_3x512
```
"""
...
from mtt.ml.data_loader import get_proc_mask
...
class MLOptimizer(
    ...
    # sandbox = dev_sandbox("bash::$MTT_BASE/sandboxes/venv_ml_plotting.sh")
    sandbox = dev_sandbox(law.config.get("analysis", "default_columnar_sandbox"))
...
# TODO: Cross check if the train weights work as intended --> by reading out process_id
class MLPreTraining(
    MTTTask,
    MLModelTrainingMixin,
    law.LocalWorkflow,
...
class MLEvaluationSingleFold(
    ...
    @law.decorator.timeit()
    def run(self):
        from mtt.ml.data_loader import MLProcessData
